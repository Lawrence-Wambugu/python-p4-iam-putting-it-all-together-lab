...
class TestRecipe:
    ...
    def test_has_attributes(self):
        ...
        with app.app_context():
            ...
            # Create user to associate with recipe
            user = User(username="Tester", _password_hash="dummyhash")
            db.session.add(user)
            db.session.commit()
            ...
            assert new_recipe.instructions.startswith("Or kind rest bred with am shed then. In")
    ...
    def test_requires_50_plus_char_instructions(self):
        '''must raise either a sqlalchemy.exc.IntegrityError or custom ValueError if instructions are too short.'''
        ...
        with app.app_context():
            ...
            user = User(username="ShortInstUser", _password_hash="shortinsthash")
            db.session.add(user)
            db.session.commit()
